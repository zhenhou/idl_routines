...
; on all fields
pro make_ptsrc_config

for ii=0, 19 do begin
    make_ptsrc_config_single_field, ii
    ;add_rk_ptsrc_to_config, ii    
endfor
end


;--------------------------------
; Add point sources to ptsrc_config* by hand
pro add_rk_all_fields
for ii=0, 19 do begin
    add_rk_ptsrc_to_config, ii
endfor
end
...
proj = 5
mytime = '20120220';date_toolkit(systime(/julian), 'file')
...
case field_name of
    ...
    'ra23h30dec-55_2008': begin ; index 1
        index_50  = [15]
        index_5snr = [119]
        ra = [357.352386]
        dec = [-49.540691]
...
; Set file names
in_fname_50mJy = in_dir + 'ptsrc_config_50mJy_'+field_name+'_'+in_date+'.txt '
in_fname_5snr  = in_dir + 'ptsrc_config_5snr_'+field_name+'_'+in_date+'.txt '

out_fname_50mJy = in_dir + 'ptsrc_config_50mJy_'+field_name+'_fixed_'+in_date+'.txt '
out_fname_5snr  = in_dir + 'ptsrc_config_5snr_'+field_name+'_fixed_'+in_date+'.txt '

; If there are no sources that need to be fixed:
if need_fix eq 0 then begin
    spawn, 'less ' + in_fname_50mJy + ' >> ' + out_fname_50mJy
    spawn, 'less ' + in_fname_5snr  + ' >> ' + out_fname_5snr
    return
endif
...
for icut=0, 1 do begin
    ...
    ; write out new file
    case icut of
        0: spawn, 'less ' + in_fname_50mJy + ' ' + tmp_fname + ' >> ' + out_fname_50mJy
        1: spawn, 'less ' + in_fname_5snr  + ' ' + tmp_fname + ' >> ' + out_fname_5snr
    endcase
endfor
