;;;
; NAME: combine_fields_ptscor.pro
; PURPOSE:
...
;  05/01/2013: (KTS) Modify for "ptscor"
;;;

PRO combine_fields_ptscor, run=run, use_kweight=use_kweight, idx_list=idx_list, $
...
case run of
    ...
    'ptscor' : combined_calib = 0.825 ; 1/1.47 = 0.680
...
;     fname = fname + '.sav'
    ...
    ; Hack for ptscor
    wh = where(idx eq [7,9,18], nwh)
    if wh ne -1 then begin
        fname=end_dir+'end_'+field+'_ptscor_kweight.sav'
    endif else begin
        fname=end_dir+'end_'+field+'_09_kweight.sav'
    end
    print,idx, ',  '+fname
