...
ncoadd = 1;0000 ; number of coadds per sim
...
signal = reform( sim_map[*, npix[1]/2] ) * 1d6 ; units of uK
...
if keyword_set(mk_sav) then save, map_name, ncoadd, nsims, signal,dl,lbins,mask_row,mask_fac,cor,sigma,n_tstream,sn_tstream,sim0,sim1,sim2,$
  filename='sig_bias_0307.sav'
if keyword_set(stopit) then stop
END
...
ss = (reform( sim_map[*, npix[1]/2] ))[500:1199]
; use bins of dl=50, l has spacing of 14, so use 4 bins; 768/4=192
nfreq=192
psd = pspec(ss, scan_speed*60.,nfreq,window='han',freqs=freqs,/psd) ; units of K/sqrt(Hz)
pp = psd^2. ; units of K^2/Hz
pp *= 1d12 ; units of uK^2/Hz
