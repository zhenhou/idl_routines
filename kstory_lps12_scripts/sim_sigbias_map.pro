...
;  03/20/2012: (KTS) Add variable scan length
;;;
...
if keyword_set(flatsky) then begin
    ...
    ;make new simulated maps each time.  It is easier to code.
    nmaps = ceil(float(nsims)/float(nx))
    print, 'nmaps = ', nmaps
    ret = dblarr(nx,nx*nmaps)

    for imap=0, nmaps-1 do begin
        if (imap mod 10 eq 0) then print, imap, format='((x, I0),$)' 
        sim_map=cmb_flatsky(ellcmb,clcmb,nx,reso*!dtor/60.)
        ret[*,imap*nx:(imap+1)*nx-1] = sim_map
    endfor
    sim_map = ret
endif
...
if keyword_set(coaddsim) then begin
    ...
    sim_map = read_dat_map('/home/kstory/lps12/sims/coaddsim_lmax8000_ra4h10dec-50.dat',nx,info.npix[1],1)
endif
...
plot, hzbins[wh], sim.psd_mb[wh,0], /ylog, title=tag+', "measured" power, 1sim',xtitle='Hz',ytitle='PSD, [uK^2/Hz]'
errplot, hzbins[wh], sim.psd_mb[wh,0]-sim.err_mb, sim.psd_mb[*,0]+sim.err_mb

plot, hzbins[wh], sim.psd_rb[wh,0], title='mean residual power, 1sim',xtitle='Hz',ytitle='PSD, [uK^2/Hz]'
errplot, hzbins[wh], sim.psd_rb[wh,0]-sim.err_rb, sim.psd_rb[*,0]+sim.err_rb
...
plot, hzbins, sim.psd_mb,/ylog,xr=[0,hzbins[wh[nwh-1]]],title='Signal and Measured power',xtitle='Hz',ytitle='PSD, [uK^2/Hz]'
oplot, hzbins, sim.psd_sb, color=!green
...
plot, hzbins,sim0.psd_rb[*,0],title='Residual power for single sim',xtitle='Hz',ytitle='psd_resid [uK^2]',xr=[0,s.hz[wh[0]]]
oplot, hzbins,sim1.psd_rb[*,0],color=!red
oplot, hzbins,sim2.psd_rb[*,0],color=!blue
...
plot, hzbins, s0.rpb,xr=xr,charsize=chsize,title=tag+', Weight 0',xtitle='Hz',ytitle='Power_resid [uK^2/Hz]'
errplot, hzbins, s0.rpb-s0.rpb_err, s0.rpb+s0.rpb_err

plot, hzbins, s1.rpb,xr=xr,charsize=chsize,title='Weight 1',xtitle='Hz',ytitle='Power_resid [uK^2/Hz]'
errplot, hzbins, s1.rpb-s1.rpb_err, s1.rpb+s1.rpb_err

plot, hzbins, s2.rpb,xr=xr,charsize=chsize,title='Weight 2',xtitle='Hz',ytitle='Power_resid [uK^2/Hz]'
...
print, 'restore, '+dir+'sim_sigbias_map_'+tag+'.sav'
restore, dir+'sim_sigbias_map_'+tag+'.sav'

sav_name_stub = dir+'sim_sigbias_map_'+tag+'_stub.sav'
...
;
;;;;;;;;;;;;;;
PRO print_summary, tag=tag, n=n, istart=istart,stopit=stopit
print, tag
dir = '/data23/kstory/lps12/sim_sigbias/'
restore, dir+'sim_sigbias_map_'+tag+'_stub.sav'
;restore, dir+'sim_sigbias_map_'+tag+'.sav'
s = sim_st

if n_elements(n) eq 0 then n = s.nsims ; number of sims to average over
if n_elements(istart) eq 0 then istart = 0
wh = indgen(n) + istart

print, ' --- Sim 0 ---'
print, 'Detect Power bias at xx sigma: ', (mean(s0.r_psd_inband[wh]) / mean(s0.s_psd_inband[wh])) / (stddev(s0.r_psd_inband[wh]) / sqrt(n) / mean(s0.s_psd_inband[wh]))
print, 'Pixel Bias [uK]  (all sims)  : ', s0.m, s0.sd/sqrt(sim_st.nsims)
print, '                           Inband                     Low-band                  High-band'
print, 'mean, err             = ', mean(s0.r_psd_inband[wh]), stddev(s0.r_psd_inband[wh])/sqrt(n), mean(s0.r_psd_low[wh]), stddev(s0.r_psd_low[wh])/sqrt(n), mean(s0.r_psd_high[wh]), stddev(s0.r_psd_high[wh])/sqrt(n)
print, 'mean_r / mean_s, err  = ', $
  mean(s0.r_psd_inband[wh]) / mean(s0.s_psd_inband[wh]), stddev(s0.r_psd_inband[wh]) / sqrt(n) / mean(s0.s_psd_inband[wh]), $
  mean(s0.r_psd_low[wh]) / mean(s0.s_psd_low[wh]), stddev(s0.r_psd_low[wh]) / sqrt(n) / mean(s0.s_psd_low[wh]), $
  mean(s0.r_psd_high[wh]) / mean(s0.s_psd_high[wh]), stddev(s0.r_psd_high[wh]) / sqrt(n) / mean(s0.s_psd_high[wh])
print, ''

print, ' --- Sim 1 ---'
print, 'Detect Power bias at xx sigma: ', (mean(s1.r_psd_inband[wh]) / mean(s1.s_psd_inband[wh])) / (stddev(s1.r_psd_inband[wh]) / sqrt(n) / mean(s1.s_psd_inband[wh]))
print, 'Pixel Bias [uK]  (all sims)  : ', s1.m, s1.sd/sqrt(sim_st.nsims)
print, '                           Inband                     Low-band                  High-band'
print, 'mean, err             = ', mean(s1.r_psd_inband[wh]), stddev(s1.r_psd_inband[wh])/sqrt(n), mean(s1.r_psd_low[wh]), stddev(s1.r_psd_low[wh])/sqrt(n), mean(s1.r_psd_high[wh]), stddev(s1.r_psd_high[wh])/sqrt(n)
print, 'mean_r / mean_s, err  = ', $
  mean(s1.r_psd_inband[wh]) / mean(s1.s_psd_inband[wh]), stddev(s1.r_psd_inband[wh]) / sqrt(n) / mean(s1.s_psd_inband[wh]), $
  mean(s1.r_psd_low[wh]) / mean(s1.s_psd_low[wh]), stddev(s1.r_psd_low[wh]) / sqrt(n) / mean(s1.s_psd_low[wh]), $
  mean(s1.r_psd_high[wh]) / mean(s1.s_psd_high[wh]), stddev(s1.r_psd_high[wh]) / sqrt(n) / mean(s1.s_psd_high[wh])
print, ''

print, ' --- Sim 2 ---'
print, 'Detect Power bias at xx sigma: ', (mean(s2.r_psd_inband[wh]) / mean(s2.s_psd_inband[wh])) / (stddev(s2.r_psd_inband[wh]) / sqrt(n) / mean(s2.s_psd_inband[wh]))
print, 'Pixel Bias [uK]  (all sims)  : ', s2.m, s2.sd/sqrt(sim_st.nsims)
print, '                           Inband                     Low-band                  High-band'
print, 'mean, err             = ', mean(s2.r_psd_inband[wh]), stddev(s2.r_psd_inband[wh])/sqrt(n), mean(s2.r_psd_low[wh]), stddev(s2.r_psd_low[wh])/sqrt(n), mean(s2.r_psd_high[wh]), stddev(s2.r_psd_high[wh])/sqrt(n)
print, 'mean_r / mean_s, err  = ', $
  mean(s2.r_psd_inband[wh]) / mean(s2.s_psd_inband[wh]), stddev(s2.r_psd_inband[wh]) / sqrt(n) / mean(s2.s_psd_inband[wh]), $
  mean(s2.r_psd_low[wh]) / mean(s2.s_psd_low[wh]), stddev(s2.r_psd_low[wh]) / sqrt(n) / mean(s2.s_psd_low[wh]), $
  mean(s2.r_psd_high[wh]) / mean(s2.s_psd_high[wh]), stddev(s2.r_psd_high[wh]) / sqrt(n) / mean(s2.s_psd_high[wh])
print, ''
...
;
;;;;;;;;;;;;;;
PRO sim_sigbias_map, ncoadd=ncoadd, nsims=nsims, npix=npix, stopit=stopit, $
                     bolo_dist=bolo_dist, $ ; use a distribution of bolo noises
                     sigfac=sigfac, sav_tag=sav_tag, verbose=verbose
...
if n_elements(sav_tag) eq 0 then print, 'Not saving any output files!'
...
if n_elements(npix) eq 0 then npix = info.npix[0]

l = make_fft_grid(reso/60.*!dtor/2./!pi,npix,fx=lx)
hz = make_fft_grid(reso/60./scan_speed,npix)
vec = indgen(npix/2 - 1)

; For binning the residuals
dl_approx = 50
max_l = max(l[vec])
nbins = floor(max_l / dl_approx)
dl = max_l / nbins
...
sim_st = {field_idx:field_idx, $  ; ra4h10dec-50
          ...
          lbins : lbins, $
          hzbins : hzbins,$
          hpfilt : complexarr(npix)}

; ------------------------
; Signal Timestream setup
; ------------------------
; if (nsims gt npix) then begin
;     print, 'nsims gt npix.  Fix this.'
;     stop
; endif
sim_map = get_sim_map(nsims,npix,/flatsky)
ss = size(sim_map)
...
wh = where(l gt 200)
tau_hz = hz[ min(wh)]
hpfilt = get_hpfilt(npix,sample_rate,tau_hz)
sim_st.hpfilt = hpfilt
...
; sim structure
tmp = {coadd_sn:fltarr(npix, nsims), $ ; coadded signal+noise timestream
       coadd_n:fltarr(npix, nsims), $ ; coadded noise timestream
       signal:fltarr(npix, nsims), $ ; signal timestream
       resid:fltarr(npix, nsims), $ ; T_meas - T_true
       wts:fltarr(ncoadd, nsims), $ ; weights
       pow_m :fltarr(npix, nsims), $ ; measured 
       pow_n :fltarr(npix, nsims), $ ; noise power
       pow_r :fltarr(npix, nsims), $ ; residual power
       pow_s :fltarr(npix, nsims), $ ; signal power
       psd_mb:fltarr(nbins, nsims), $ ; psd [uK^2/Hz], binned
       psd_nb:fltarr(nbins, nsims), $ ; psd [uK^2/Hz], binned
       psd_rb:fltarr(nbins, nsims), $ ; psd [uK^2/Hz], binned
       psd_sb:fltarr(nbins, nsims), $ ; psd [uK^2/Hz], binned
...
n_tstream = fltarr(npix, ncoadd) ; noise timestreams
sn_tstream = fltarr(npix, ncoadd) ; signal+noise timestreams
...
for isim=0L, nsims-1 do begin
    ...
    for jj=0L, ncoadd-1 do begin
        undefine, seed
        noise_tmp = randomn(seed, npix) * sigma[jj,isim]
    ...
    for ii=0, nbins-1 do begin
        wh = where(l ge dl*ii and l lt dl*(ii+1))
        sim0.psd_mb[ii,isim] = total(sim0.pow_m[wh,isim])/dHz
        sim0.psd_nb[ii,isim] = total(sim0.pow_n[wh,isim])/dHz
        sim0.psd_rb[ii,isim] = total(sim0.pow_r[wh,isim])/dHz
        sim1.psd_mb[ii,isim] = total(sim1.pow_m[wh,isim])/dHz
        sim1.psd_nb[ii,isim] = total(sim1.pow_n[wh,isim])/dHz
        sim1.psd_rb[ii,isim] = total(sim1.pow_r[wh,isim])/dHz
        sim2.psd_mb[ii,isim] = total(sim2.pow_m[wh,isim])/dHz
        sim2.psd_nb[ii,isim] = total(sim2.pow_n[wh,isim])/dHz
        sim2.psd_rb[ii,isim] = total(sim2.pow_r[wh,isim])/dHz
    end
...
for ii=0, nbins-1 do begin
    sim0.err_mb[ii] = stddev(sim0.psd_mb[ii,*])
    sim0.err_nb[ii] = stddev(sim0.psd_nb[ii,*])
    sim0.err_rb[ii] = stddev(sim0.psd_rb[ii,*])
    sim1.err_mb[ii] = stddev(sim1.psd_mb[ii,*])
    sim1.err_nb[ii] = stddev(sim1.psd_nb[ii,*])
    sim1.err_rb[ii] = stddev(sim1.psd_rb[ii,*])
    sim2.err_mb[ii] = stddev(sim2.psd_mb[ii,*])
    sim2.err_nb[ii] = stddev(sim2.psd_nb[ii,*])
    sim2.err_rb[ii] = stddev(sim2.psd_rb[ii,*])
endfor
...
for jj=0L, nsims-1 do begin
    sim0.chisq_rb[jj] = total( (sim0.psd_rb[wh,jj]/sim0.err_rb[wh])^2.)
    sim0.pte_rb[jj]   = mpchitest(sim0.chisq_rb[jj], nwh)
    sim1.chisq_rb[jj] = total( (sim1.psd_rb[wh,jj]/sim1.err_rb[wh])^2.)
    sim1.pte_rb[jj]   = mpchitest(sim1.chisq_rb[jj], nwh)
    sim2.chisq_rb[jj] = total( (sim2.psd_rb[wh,jj]/sim2.err_rb[wh])^2.)
...
for j=0L,nsims-1 do begin
    s0.r_psd_inband[j] = total(sim0.psd_rb[wh_1to3,j]) * dHz / nHz_inband
    s0.r_psd_low[j]    = total(sim0.psd_rb[wh_low,j])  * dHz / nHz_low
    s0.r_psd_high[j]   = total(sim0.psd_rb[wh_high,j]) * dHz / nHz_high

    s0.s_psd_inband[j] = total(sim0.psd_sb[wh_1to3,j]) * dHz / nHz_inband
    s0.s_psd_low[j]    = total(sim0.psd_sb[wh_low,j])  * dHz / nHz_low
    s0.s_psd_high[j]   = total(sim0.psd_sb[wh_high,j]) * dHz / nHz_high

    s0.n_psd_inband[j] = total(sim0.psd_nb[wh_1to3,j]) * dHz / nHz_inband
    s0.n_psd_low[j]    = total(sim0.psd_nb[wh_low,j])  * dHz / nHz_low
    s0.n_psd_high[j]   = total(sim0.psd_nb[wh_high,j]) * dHz / nHz_high


    s1.r_psd_inband[j] = total(sim1.psd_rb[wh_1to3,j]) * dHz / nHz_inband
    s1.r_psd_low[j]    = total(sim1.psd_rb[wh_low,j])  * dHz / nHz_low
    s1.r_psd_high[j]   = total(sim1.psd_rb[wh_high,j]) * dHz / nHz_high

    s1.s_psd_inband[j] = total(sim1.psd_sb[wh_1to3,j]) * dHz / nHz_inband
    s1.s_psd_low[j]    = total(sim1.psd_sb[wh_low,j])  * dHz / nHz_low
    s1.s_psd_high[j]   = total(sim1.psd_sb[wh_high,j]) * dHz / nHz_high

    s1.n_psd_inband[j] = total(sim1.psd_nb[wh_1to3,j]) * dHz / nHz_inband
    s1.n_psd_low[j]    = total(sim1.psd_nb[wh_low,j])  * dHz / nHz_low
    s1.n_psd_high[j]   = total(sim1.psd_nb[wh_high,j]) * dHz / nHz_high


    s2.r_psd_inband[j] = total(sim2.psd_rb[wh_1to3,j]) * dHz / nHz_inband
    s2.r_psd_low[j]    = total(sim2.psd_rb[wh_low,j])  * dHz / nHz_low
    s2.r_psd_high[j]   = total(sim2.psd_rb[wh_high,j]) * dHz / nHz_high

    s2.s_psd_inband[j] = total(sim2.psd_sb[wh_1to3,j]) * dHz / nHz_inband
    s2.s_psd_low[j]    = total(sim2.psd_sb[wh_low,j])  * dHz / nHz_low
    s2.s_psd_high[j]   = total(sim2.psd_sb[wh_high,j]) * dHz / nHz_high

    s2.n_psd_inband[j] = total(sim2.psd_nb[wh_1to3,j]) * dHz / nHz_inband
    s2.n_psd_low[j]    = total(sim2.psd_nb[wh_low,j])  * dHz / nHz_low
    s2.n_psd_high[j]   = total(sim2.psd_nb[wh_high,j]) * dHz / nHz_high
endfor

for i=0, nbins-1 do begin
    s0.rpb[i] = mean(sim0.psd_rb[i,0:nsims-1])
    s1.rpb[i] = mean(sim1.psd_rb[i,0:nsims-1])
    s2.rpb[i] = mean(sim2.psd_rb[i,0:nsims-1])
endfor
...
if n_elements(sav_tag) eq 0 then begin
...
dir = '/data23/kstory/lps12/sim_sigbias/'
if n_elements(sav_tag) ne 0 then save, sim_st,wh_1to3,hz,mask_row,whmask,mask_fac,cor,hpfilt,sigma,n_tstream,sn_tstream,pn,$
  sim0,sim1,sim2,s0,s1,s2,$
  filename=dir+'sim_sigbias_map_'+tag+'.sav'
if keyword_set(stopit) then stop
END
