...
; on all fields
pro make_ptsrc_config_0321

for ii=0, 20 do begin
    make_ptsrc_config_single_field, ii, $
      outdir = '/home/kstory/lps12/ptsrc_lists/20120321/'
endfor
end
...
; INPUTS: outdir,   directory where .txt files will be stored
pro make_ptsrc_config_single_field, field_idx, $
                                    outdir = outdir

if ~keyword_set(outdir) then outdir = '/home/kstory/lps12/ptsrc_lists/'
...
proj = 5
mydate = strsplit(date_toolkit(systime(/julian), 'file'), '_', /extract) ; get current day to tag file
...
configfile_out_50mJy  = outdir+'ptsrc_config_50mJy_'+field_name+'_'+mydate+'.txt'
...
configfile_out_6p4mJy  = outdir+'ptsrc_config_6p4mJy_'+field_name+'_'+mydate+'.txt'
