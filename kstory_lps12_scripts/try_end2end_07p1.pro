...
PRO try_end2end_07p1, field_idx, $
                    stopit=stopit, $
...
;-------------------------
; get MASK
;    Let kernel get re-made
;-------------------------
mask = get_lps12_mask(field_idx)
ss = size(mask)
npad = 2160L
mask_padded = dblarr(npad,npad)
mask_padded[0:ss[1]-1, 0:ss[2]-1] = mask
...
;    kmask = get_lps12_kweight(field_idx)
    restore, '/home/kstory/lps12/end2end/run_07p1/rktmp_weight_2d_07p1.sav'
    kmask=weight_2d
endif
