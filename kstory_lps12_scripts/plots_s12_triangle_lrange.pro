...
; PURPOSE:
;   Make a triangle plot of the lrange test
;
; NOTES:
;;;
...
filename_stub = '~kstory/lps12/scripts/figs/triangle_lcdm_lrange'
...
mycolor = [color_lmx15, color_lmx30, color_w7s12] ; [SPT-only, WMAP-only, SPT+WMAP]
...
for i=0, np-1 do begin ; row, top to bottom
    ...
    for k=0,i do begin ; column
        ...
        if k eq i then begin    ; diagonal
            plot_like1dname,files_lmx15,pname_lmx15,p[k].n,subsamp=3,nskip=1000,scale=p[k].scale,$
              thick=4,linestyle=ls[0], color=mycolor[0], /oplot
            plot_like1dname,files_lmx30,pname_lmx30,p[k].n,subsamp=3,nskip=1000,scale=p[k].scale,$
            ...
            plot_like2dname,files_lmx15,pname_lmx15,p[k].n,p[i].n,scale1=p[k].scale,scale2=p[i].scale,$
              /overplot,sigma=[1,2],$
              icolors=[pscolors[5],pscolors[8]],subsamp=3,yl=0 ; forest, blue
            plot_like2dname,files_lmx30,pname_lmx30,p[k].n,p[i].n,scale1=p[k].scale,scale2=p[i].scale,$
              /overplot,sigma=[1,2],$
              icolors=[pscolors[10],pscolors[3]],subsamp=3,yl=0 ; purple, orange
...
t1=systime(/seconds)
print, 'This procedure took ', (t1-t0)/60.,' minutes'
stop
END
