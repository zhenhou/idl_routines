function get_2dplane_weight, p1_list, p2_list, weight, nbins_raw, frate, $
xaxis_zero=xaxis_zero, yaxis_zero=yaxis_zero, nointerp=nointerp

    nsteps = n_elements(p1_list)
    if (n_elements(p2_list) ne nsteps) then begin
        print, 'p1_list and p2_list should have the same length.'
        stop
    endif

    xmin1 = min(p1_list, max=xmax1)
    xmin2 = min(p2_list, max=xmax2)
    ...
    plane = dblarr(nbins,nbins)

    y1raw = long((p1_list-xmin1-0.5*dx1)/dx1)
    y2raw = long((p2_list-xmin2-0.5*dx2)/dx2)

    for i=0L, nsteps-1L do begin
       plane[y1raw[i],y2raw[i]] += weight[i]
    endfor
