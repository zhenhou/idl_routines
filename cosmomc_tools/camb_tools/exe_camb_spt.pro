pro exe_camb_spt, params, output_root, cls, old_camb=old_camb, camb_path=camb_path, pivot_k=pivot_k, add=add

    homedir = '/home/hou/'
    
    if (not keyword_set(camb_path)) then camb_path = homedir+'Projects/CMBtools/cosmologist.info/camb/'
    camb = camb_path+'/camb'
    ...
    fmt = '(A,D16.7)'
    ini_file = homedir+'tmp/'+output_root+'.ini'
    get_lun, unit_ini
    ...
    endelse
    printf, unit_ini, 'output_root     = '+homedir+'tmp/'+output_root
    ...
    printf, unit_ini, format=fmt, 'Alens = ', alens

    tags = tag_names(params)
    ipadd = where(tags eq strupcase('more_pnames'))
    if (ipadd[0] ne -1) then begin
        num_add_params = n_elements(params.more_pnames)
        printf, unit_ini, ' '
        for i_add=0, num_add_params-1 do begin
            printf, unit_ini, format=fmt, params.more_pnames[i_add]+' = ', params.more_params[i_add]
        endfor
    endif
    ...
    lensedcls_file = homedir+'tmp/'+output_root+'_lensedCls.dat'
    ...
    scalcls_file = homedir+'tmp/'+output_root+'_lenspotentialCls.dat'
    ...
    spawn, 'rm -rf '+homedir+'tmp/'+output_root+'_*'
